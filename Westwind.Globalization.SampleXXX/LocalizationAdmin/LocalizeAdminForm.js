...
$(document).ready(function() {
    ...
    txtResSearch.keyup(function (e) {
        var entered = txtResSearch.val().toLowerCase();
        var list = lstResourceIds.get(0);

        showStatus(e.keyCode);

        if (e.keyCode == 27) {
            $(this).val("");
            $("#lstResourceIds option").show();
            e.preventDefault();
            e.stopPropagation();
            return false;
        }
...
// Custom containsNoCase jQuery Filter
$.expr[":"].containsNoCase = function (el, i, m) {
    var search = m[3]; // filter parm list as string
    if (!search) return false;
    //return eval("/" + search + "/i").test($(el).text());
    // the following relies on ww.jquery.js to escape regEx expr
    return eval("/" + search.escapeRegExp() + "/i").test($(el).text());
};